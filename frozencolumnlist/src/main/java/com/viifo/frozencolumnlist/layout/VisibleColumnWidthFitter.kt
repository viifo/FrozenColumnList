...
/** 为 START / END 模式按可用宽度计算普通列宽，并在尺寸变化时自动重新适配。 */
internal object VisibleColumnWidthFitter {
    ...
    private class Binding(
        ...
        private var fallbackViewportWidth: Int? = null
        ...
        fun update(
            ...
            fallbackViewportWidth = viewportWidth?.takeIf { it > 0 }
            fit()
        }
        ...
        private fun fit() {
            val viewportWidth = row.width.takeIf { it > 0 } ?: fallbackViewportWidth ?: return
            ...
            val availableWidth = (
                viewportWidth - row.paddingLeft - row.paddingRight - frozenWidth
                ).coerceAtLeast(0)
